...
<script lang="ts">
    ...
    function viewJudgeDetails(proof: ReputationProof) {
        ...
        judge_detail.set({ proof, address });
    }
</script>
...
<div class="container mx-auto p-4 md:p-8">
    <div class="text-center mb-10">
        ...
        <p class="text-muted-foreground mt-2">
            Browse all on-chain judge profiles in the ecosystem.
        </p>
    </div>
    ...
    {:else if error}
        <div
            class="text-center text-red-500 bg-red-50 dark:bg-red-900/20 p-4 rounded-md"
        >
            {error}
        </div>
    ...
    {:else}
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {#each Array.from($judges.data.values()) as proof (proof.token_id)}
                <div
                    class="border rounded-lg p-5 shadow-sm hover:shadow-md transition-shadow bg-card"
                >
                    <h3
                        class="font-semibold text-lg truncate"
                        title={proof.token_id}
                    >
                        Judge ID: {proof.token_id.substring(0, 12)}...
                    </h3>
                    <div class="mt-3 text-sm text-muted-foreground space-y-2">
                        <p>
                            <strong>Opinions Issued:</strong>
                            {proof.number_of_boxes}
                        </p>
                        <p>
                            <strong>Reputation Stake:</strong>
                            {total_burned_string(proof)} ERG
                        </p>
                    </div>
                    <div class="mt-4">
                        <Button
                            on:click={() => viewJudgeDetails(proof)}
                            variant="outline"
                            size="sm"
                        >
                            View Details
